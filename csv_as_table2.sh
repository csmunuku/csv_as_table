...
max_field_length()
{
  ...
  awk -v fieldNum=${1} -F, 'BEGIN{maxlen=0;} {len=length($fieldNum);maxlen=maxlen>len?maxlen:len;} END{print maxlen}' ${2}
}
...
csv_to_table()
{
  if [[ $# -eq 1 ]]; then
     ...
     do
       ...
       echo
       count=0
       if [[ ${mycount} -ne $(wc -l ${1} | cut -d' ' -f1) ]]; then
          for i in $(echo $line | sed 's/,/ /g')
          do
            curr_field_length=
            curr_field_length=$(echo ${i} | awk '{print length($0)}')
            ...
            my_field=field_${count}
            my_field_len=$(max_field_length ${count} ${1})
            print_field_ln $count ${my_field_len}
            if [[ ${count} -eq ${fields_in_csv} ]]; then
                echo -n "+"
                echo
            fi
          done
